...
├──────────────────────────────────────────────────────────────┤
│                      AI Runtime Layer                        │
│  DeepSeek Client       Tool Definitions      Loop Engine   │
...
- **client.ts**: DeepSeek API client wrapper (uses the Anthropic-compatible endpoint at `https://api.deepseek.com/anthropic`)
...
AI Runtime (loop.ts)
    │
    ▼
DeepSeek API (client.ts) ──► Tool Call: task.create
    │
    ▼
...
│                                                          │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────┐  │
│  │  Loop Engine │───►│ DeepSeek API │◄───│  Tool Defs  │  │
...
3. Sends a request to the DeepSeek API (Anthropic-compatible endpoint) with tool definitions

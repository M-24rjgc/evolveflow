...
function timeSeed(): number {
  const now = new Date();
  return now.getFullYear() * 1000000 + now.getMonth() * 100000 + now.getDate() * 10000 + now.getHours() * 60 + now.getMinutes();
}
...
export class BuddyCore {
  ...
  generateScheduleComment(taskCount: number): string | null {
    ...
    const message = typeof template === 'function'
      ? (template as (n: number) => string)(taskCount)
      : template;
  ...
  inferMood(completedRatio: number, pendingCount: number, isLate: boolean): BuddyMood {
    if (isLate || pendingCount > 10) {return 'concerned';}
    if (completedRatio >= 0.8) {return 'happy';}
    if (completedRatio >= 0.5) {return 'encouraging';}
    if (completedRatio >= 0.3) {return 'neutral';}
    return 'concerned';
  }
  ...
  applyDreamInsights(adjustments: BuddyAdjustments): void {
    if (adjustments.encouragementFrequency !== undefined) {
      this.personality.encouragementFrequency = Math.max(0, Math.min(1, adjustments.encouragementFrequency));
    }
    ...
    console.log(`[Buddy] Applied dream adjustments: freq=${this.personality.encouragementFrequency.toFixed(2)}, ` +
  ...
  private applyPersonalityToText(text: string): string {
    ...
    if (this.personality.formality >= 0.7) {
      result = result
        .replace(/[\u{1F600}-\u{1F64F}\u{1F389}\u{2728}\u{1F4AA}\u{1F680}\u{1F31F}\u{1F4A5}\u{1F3AF}\u{1F44F}\u{1F44D}\u{1F49A}\u{1F33F}\u{1F331}\u{1F38A}\u{1F389}]/gu, '') // Strip emoji
        .replace(/[～~]+/g, '。')
        .replace(/！+/g, '。')
        .replace(/[？?]+/g, '。')
        .replace(/[。]{2,}/g, '。')

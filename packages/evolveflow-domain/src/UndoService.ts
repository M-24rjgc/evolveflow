...
export class UndoService {
  ...
  revertAction(actionLogId: string): boolean {
    ...
    const stateBefore = actionLog.state_before
      ? (JSON.parse(actionLog.state_before) as Record<string, unknown>)
      : null;
    const stateAfter = actionLog.state_after
      ? (JSON.parse(actionLog.state_after) as Record<string, unknown>)
      : null;
    ...
    const revertTransaction = this.db.transaction(() => {
      if (capability.startsWith('task.')) {
        ...
        if (!stateBefore) {
          throw new Error(`Cannot revert action: no state_before snapshot`);
        }
        this.revertScheduleAction(actionLogId, stateBefore);
      } else if (capability.startsWith('reminder.')) {
        if (!stateBefore) {
          throw new Error(`Cannot revert ${capability}: no state_before snapshot`);
        }
        ...
        throw new Error(
          `Unsupported undo capability: ${capability}. No revert handler registered for this capability type.`
        );
      }
      ...
      this.db
        .prepare('INSERT INTO undo_groups (id, description, created_at) VALUES (?, ?, ?)')
        .run(undoGroupId, `Reverted: ${capability}`, now);
      this.db
        .prepare('UPDATE action_logs SET undo_group_id = ? WHERE id = ?')
        .run(undoGroupId, actionLogId);
  ...
  private revertTaskAction(
    ...
    if (capability === 'task.create') {
      ...
      if (!taskId) {
        throw new Error(`Cannot revert task.create: no created task id`);
      }
      ...
      capability === 'task.defer' ||
      capability === 'task.lock'
    ) {
      if (!stateBefore) {
        throw new Error(`Cannot revert ${capability}: no state_before snapshot`);
      }
  ...
  private revertEventAction(
    ...
    if (capability === 'event.create') {
      ...
      if (!eventId) {
        throw new Error(`Cannot revert event.create: no created event id`);
      }
  ...
  private revertReminderAction(capability: string, stateBefore: Record<string, unknown>): void {
    const now = new Date().toISOString();
    if (
      capability === 'reminder.dismiss' ||
      capability === 'reminder.snooze' ||
      capability === 'reminder.create'
    ) {
      const reminderId = stateBefore.id as string;
      if (!reminderId) {
        throw new Error(`Cannot revert ${capability}: no reminder id in state_before`);
      }
  ...
  private revertMemoryAction(capability: string, stateBefore: Record<string, unknown>): void {
    ...
    if (!memoryId) {
      throw new Error(`Cannot revert ${capability}: no memory id in state_before`);
    }
    // Restore all fields from stateBefore, excluding updated_at
    const fields = Object.keys(stateBefore).filter((k) => k !== 'updated_at');
    const sets = fields.map((k) => `${k} = ?`).join(', ');
    ...
    this.db
      .prepare(`UPDATE dream_insights SET ${sets}, updated_at = ? WHERE id = ?`)
      .run(...values);
  }
  ...
  private revertScheduleAction(actionLogId: string, stateBefore: Record<string, unknown>): void {
    ...
    if (date) {
      this.restoreScheduleDateSnapshot(actionLogId, date, stateBefore.blocks);
      return;
    }

    const dates = Array.isArray(stateBefore.dates) ? stateBefore.dates : [];
    const blocksByDate = stateBefore.blocksByDate as Record<string, unknown> | undefined;
    if (dates.length > 0 && blocksByDate) {
      for (const item of dates) {
        if (typeof item !== 'string') {
          continue;
        }
        this.restoreScheduleDateSnapshot(actionLogId, item, blocksByDate[item]);
      }
      return;
    }

    throw new Error(`Cannot revert schedule action ${actionLogId}: no schedule snapshot`);
  }

  private restoreScheduleDateSnapshot(actionLogId: string, date: string, rawBlocks: unknown): void {
    const now = new Date().toISOString();
    let blocks: Record<string, unknown>[] = [];

    if (rawBlocks) {
      try {
        blocks = JSON.parse(rawBlocks as string) as Record<string, unknown>[];
      } catch (e) {
        throw new Error(
          `Cannot revert schedule action ${actionLogId}: invalid JSON in stateBefore.blocks: ${(e as Error).message}`
        );
      }
    }

    this.db.prepare('DELETE FROM schedule_blocks WHERE date = ?').run(date);
    for (const block of blocks) {
      ...
      `
        ...
        .run(
          ...
          typeof block.manual_signal === 'boolean'
            ? block.manual_signal
              ? 1
              : 0
            : block.manual_signal,

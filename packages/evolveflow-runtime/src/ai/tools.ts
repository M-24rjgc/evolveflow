...
import type { CapabilityDefinition, CapabilityRegistry } from '@evolveflow/capabilities';
...
/** Get system prompt text listing available tools */
export function getToolListingPrompt(
  registry: CapabilityRegistry,
  options: { include?: (capability: CapabilityDefinition) => boolean } = {}
): string {
  const capabilities = options.include ? registry.list().filter(options.include) : registry.list();

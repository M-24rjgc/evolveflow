...
export async function* runConversation(
  ...
  while (turnCount < maxTurns) {
    ...
    const fullSystemPrompt = buildSystemPrompt(systemPrompt, context, mode);
      ...
      for (const [, toolBlock] of currentToolBlocks) {
        ...
        try {
          const result = await registry.invoke(capabilityName, parsedInput, {
            ...
            origin: toolOrigin,
            session_id: sessionId,
            // Mutating tool calls are deduplicated by the tool_use id so that
            // if a single tool_use block is executed more than once (e.g. due
            // to an outer retry), it does not create duplicate state. The
            // model guarantees tool_use ids are unique, making them a safe
            // idempotency key.
            ...(mutating ? { idempotency_key: `ai:${sessionId}:${toolBlock.id}` } : {}),
          });
...
function buildContextBlock(context: ConversationContext, mode: AgentMode): SystemMessageParam[] {
  ...
  lines.push('</evolveflow_context>');
  lines.push(getContextInstruction(mode));
...
function getContextInstruction(mode: AgentMode): string {
  if (mode === 'chat') {
    return [
      '',
      '这是一个只读上下文快照。你不能调用工具，不能声称正在查询、检查、读取、写入或修改本地数据。',
      '可以基于快照中已经给出的信息自然回答；如果信息不足，直接说明当前信息有限，并请用户补充或切换到计划/自动模式。',
      '不要主动执行排程优化或暗示后台正在处理任务。',
    ].join('\n');
  }
  ...
  if (mode === 'plan') {
    return [
      '',
      '请基于以上上下文帮助用户分析日程、制定计划和解释取舍。',
      '如需核对当前数据，可以使用可用的只读工具查询；不要创建、修改、删除、恢复数据或运行会改变系统状态的命令。',
      '如果用户需要实际变更，请给出明确方案并提示切换到自动模式执行。',
    ].join('\n');
  }
...
/**
 * Collect the set of tool_use ids carried by a message's content blocks.
 */
function collectToolUseIds(message: MessageParam): Set<string> {
...
/**
 * Collect the set of tool_use ids that a message's tool_result blocks answer.
 */
...
/**
 * Adjust a candidate split index forward (towards higher indices) so that the
 * lower segment `[0, split)` does not end with an assistant message whose
 * tool_use blocks are answered only by messages after the split. The Anthropic
 * API requires every tool_use to be immediately followed by its tool_result;
 * cutting between them produces a 400 error.
 *
 * Returns a split index in `(0, total]`.
 */
function adjustSplitForToolPairing(messages: MessageParam[], candidateSplit: number): number {
  const total = messages.length;
  let split = Math.max(1, Math.min(candidateSplit, total));

  // Gather all tool_use ids seen below the split.
  const seenToolUseIds = new Set<string>();
  for (let i = 0; i < split; i++) {
    for (const id of collectToolUseIds(messages[i])) {
      seenToolUseIds.add(id);
    }
  }
  // Gather all tool_result ids answered below the split.
  const answeredToolUseIds = new Set<string>();
  for (let i = 0; i < split; i++) {
    for (const id of collectToolResultIds(messages[i])) {
      answeredToolUseIds.add(id);
    }
  }

  // While there is an unanswered tool_use below the split, pull the split
  // forward until that tool_use's matching result is included.
  // Guard against pathological loops with a hard cap.
  let guard = 0;
  while (guard++ < total + 2) {
    const pending = new Set<string>();
    for (const id of seenToolUseIds) {
      if (!answeredToolUseIds.has(id)) {
        pending.add(id);
      }
    }
    if (pending.size === 0 || split >= total) {
      break;
    }
    // Move split forward to include the next message, then update both sets.
    const incoming = messages[split];
    if (!incoming) {
      break;
    }
    for (const id of collectToolUseIds(incoming)) {
      seenToolUseIds.add(id);
    }
    for (const id of collectToolResultIds(incoming)) {
      answeredToolUseIds.add(id);
    }
    split++;
  }
...
/**
 * Split messages into (first, last) segments around a middle that will be
 * summarized. Both split boundaries are adjusted so neither segment leaves a
 * tool_use without its tool_result. The middle may end up empty, in which
 * case the caller should skip summarization.
 */
function splitForCompaction(
  messages: MessageParam[],
  keepFirst: number,
  keepLast: number
): { first: MessageParam[]; middle: MessageParam[]; last: MessageParam[] } {
  if (messages.length <= keepFirst + keepLast) {
    return { first: messages, middle: [], last: [] };
  }

  // Raw candidate boundaries.
  let headEnd = keepFirst; // exclusive end of first segment
  const tailStartCandidate = messages.length - keepLast; // start of last segment

  // Adjust head boundary forward so no tool_use is left dangling.
  headEnd = adjustSplitForToolPairing(messages, headEnd);

  // For the tail boundary we work on the reversed array: a tool_result at the
  // start of the tail is "dangling" if its tool_use sits in the middle/first.
  // We mirror the same logic by adjusting the tail start forward (towards
  // higher indices) until the first segment of the tail no longer references a
  // tool_use id that lives before the tail.
  let tailStart = tailStartCandidate;
  const guard = messages.length + 2;
  for (let i = 0; i < guard && tailStart < messages.length; i++) {
    // tool_use ids known at/after tailStart
    const tailToolUseIds = new Set<string>();
    for (let j = tailStart; j < messages.length; j++) {
      for (const id of collectToolUseIds(messages[j])) {
        tailToolUseIds.add(id);
      }
    }
    // tool_result ids referenced at/after tailStart that are NOT produced by a
    // tool_use at/after tailStart → these need their producing assistant msg,
    // which means we must shrink the middle by moving tailStart down.
    const orphanedResultIds = new Set<string>();
    for (let j = tailStart; j < messages.length; j++) {
      for (const id of collectToolResultIds(messages[j])) {
        if (!tailToolUseIds.has(id)) {
  ...
  if (headEnd >= tailStart) {
    // Boundaries collapsed onto each other: nothing safe to summarize.
    return { first: messages.slice(0, headEnd), middle: [], last: messages.slice(headEnd) };
  }

  return {
    first: messages.slice(0, headEnd),
    middle: messages.slice(headEnd, tailStart),
    last: messages.slice(tailStart),
  };
}
...
function compactConversation(session: AiSessionState): void {
  ...
  const { first, middle, last } = splitForCompaction(session.messages, 2, 4);

  if (middle.length === 0) {
    // Nothing safe to summarize without breaking tool pairing; bail out.
    return;
  }

  const summary = buildCompactSummary(middle, middle.length);
  ...
  session.messages = [...first, summaryMessage, ...last];

  // Validate message alternation — repair consecutive user messages.
  // (Role alternation is a weaker invariant than tool pairing, which was
  // already preserved by splitForCompaction above.)
...
function aggressiveCompact(session: AiSessionState): void {
  ...
  const { first, middle, last } = splitForCompaction(session.messages, 2, 2);
  ...
  const aggressiveSummary: MessageParam = {
    role: 'user',
    content: `[紧急压缩: ${middle.length}条消息已合并。${stripped}]`,
  };

  session.messages = [...first, aggressiveSummary, ...last];
  ensureMessageAlternation(session.messages);
}
...
// Exported for direct unit testing of tool-pairing preservation during
// conversation compaction (see runtime/tests/ai.test.ts).
export { compactConversation, aggressiveCompact, splitForCompaction };
